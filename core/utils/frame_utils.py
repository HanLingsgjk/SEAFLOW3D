...
def read_vkitti_png_flow(flow_fn):
    ...
    # Set invalid flow regions to 0 (or another value like np.nan)
    out_flow[invalid] = 0

    return out_flow,invalid==0
...
def readFlowFFD(filename):
    ...
    return flow, valid
def readDC(filename):
    dc = cv2.imread(filename, cv2.IMREAD_ANYDEPTH|cv2.IMREAD_COLOR)
...
def readNerfMask(filename):
    mask = cv2.imread(filename, cv2.IMREAD_ANYDEPTH|cv2.IMREAD_COLOR)
    mask = mask.astype(np.float32)
    ssim = mask[:, :,0]/16384.0
    d2dloss = mask[:, :,1]/16384.0
    alpha = mask[:, :,2]/16384.0

    return ssim,d2dloss,alpha

def readNerfddc(filename):
    mask = cv2.imread(filename, cv2.IMREAD_ANYDEPTH|cv2.IMREAD_COLOR)
    mask = mask.astype(np.float32)
    d1 = mask[:, :,0]/1024.0
    d2 = mask[:, :,1]/1024.0

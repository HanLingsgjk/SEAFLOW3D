...
class Final_UnetV1(nn.Module):
    def __init__(self, inplanes, outplanes):
        super(Final_UnetV1, self).__init__()
        self.proj = conv2DBatchNormRelu(in_channels=inplanes, k_size=1, n_filters=64, padding=0, stride=1)
        self.inplanes = 64
        # Vanilla Residual Blocks
        self.res_block3 = self._make_layer(residualBlock, 64, 1, stride=2)
        ...
        self.iconv3 = conv2DBatchNormRelu(in_channels=96, k_size=3, n_filters=64,
                                          padding=1, stride=1)
        self.iconv2E = nn.Sequential(conv2DBatchNormRelu(in_channels=96, k_size=3, n_filters=64,
                                                        padding=1, stride=1),
                                    nn.Conv2d(64, outplanes, kernel_size=3, stride=1, padding=1, bias=True))
    ...
    def forward(self, x):
        ...
        conv3u = F.upsample(conv3, [x.size()[2], x.size()[3]], mode='bilinear')
        concat2 = torch.cat((proj, self.upconv3(conv3u)), dim=1)
        pred2E = self.iconv2E(concat2)  # 4x
        return pred2E
#这个是一个专门用于光流迭代卷积的模块
class Tiny_Unetlike(nn.Module):
    def __init__(self, inplanes, outplanes):
        super(Tiny_Unetlike, self).__init__()
        self.proj = conv2DBatchNormRelu(in_channels=inplanes, k_size=1, n_filters=128, padding=0, stride=1)
        self.inplanes = 128
        # Vanilla Residual Blocks
        self.res_block3 = self._make_layer(residualBlock, 128, 1, stride=2)
        self.res_block5 = self._make_layer(residualBlock, 128, 1, stride=2)
        self.pyramid_pooling = pyramidPooling(128, levels=3)
        # Iconvs
        self.upconv6 = conv2DBatchNormRelu(in_channels=128, k_size=3, n_filters=128,
                                           padding=1, stride=1)
        self.upconv5 = conv2DBatchNormRelu(in_channels=128, k_size=3, n_filters=64,
                                           padding=1, stride=1)


        self.iconv5 = conv2DBatchNormRelu(in_channels=256, k_size=3, n_filters=128,
                                          padding=1, stride=1)
        self.iconv4 = conv2DBatchNormRelu(in_channels=192, k_size=3, n_filters=128,
                                          padding=1, stride=1)
...
class Tiny_Unetlike(nn.Module):
    ...
    def forward(self, x):
        ...
        return pred6
#使用Convnext强化卷积范围
class Tiny_Unetlikev7(nn.Module):#拿这个出一个上采样
    ...
    def forward(self, x):
        ...
        return pred6
#使用Convnext强化卷积范围
class Tiny_Unetlikev7t(nn.Module):#拿这个出一个上采样
    def __init__(self, inplanes, outplanes,mid_channle = 128):
        super(Tiny_Unetlikev7t, self).__init__()
        self.proj = conv2DBatchNormRelu(in_channels=inplanes, k_size=1, n_filters=mid_channle, padding=0, stride=1)
        #self.proj = ConvNeXtV2tiny(inplanes, mid_channle,bl=2)
        self.inplanes = mid_channle
        # Vanilla Residual Blocks
    ...
    def forward(self, x):
        ...
        return pred6
class Tiny_UnetDRU(nn.Module):#拿这个出一个上采样
    def __init__(self, inplanes, outplanes,mid_channle = 128):
        super(Tiny_UnetDRU, self).__init__()
        #self.proj = conv2DBatchNormRelu(in_channels=inplanes, k_size=1, n_filters=mid_channle, padding=0, stride=1)
        self.proj = ConvNeXtV2tiny(inplanes, mid_channle,bl=2)
        self.inplanes = mid_channle
        # Vanilla Residual Blocks
...
class Tiny_Unetlikev2(nn.Module):
    ...
    # 使用Convnext强化卷积范围
class Tiny_Unetlikev8(nn.Module):
        def __init__(self, inplanes, outplanes, mid_channle=128):
            super(Tiny_Unetlikev8, self).__init__()
            # self.proj = conv2DBatchNormRelu(in_channels=inplanes, k_size=1, n_filters=mid_channle, padding=0, stride=1)
            self.proj = ConvNeXtV2tiny(inplanes, mid_channle, bl=2)
            self.inplanes = mid_channle
            # Vanilla Residual Blocks
            ...
            self.iconv5 = conv2DBatchNormRelu(in_channels=self.inplanes * 4, k_size=3, n_filters=self.inplanes,
                                              padding=1, stride=1)
            self.iconv4 = conv2DBatchNormRelu(in_channels=self.inplanes + 64, k_size=3, n_filters=self.inplanes,
                                              padding=1, stride=1)
        ...
        def forward(self, x,cx16):
            ...
            conv6u = F.upsample(conv6, [rconv3.size()[2], rconv3.size()[3]], mode='bilinear')
            concat5 = torch.cat((cx16,rconv3, self.upconv6(conv6u)), dim=1)
            conv5 = self.iconv5(concat5)  # 32x
            ...
            return pred6
#使用Convnext强化卷积范围
class Tiny_Unetlikev3(nn.Module):
    def __init__(self, inplanes, outplanes,mid_channle = 128):
        super(Tiny_Unetlikev3, self).__init__()
        self.proj0 = conv2DBatchNormRelu(in_channels=inplanes, k_size=1, n_filters=mid_channle, padding=0, stride=1)
        self.inplanes = mid_channle
        # Vanilla Residual Blocks
        self.res_block3 = ConvNeXtV2tiny(self.inplanes,self.inplanes,down=True)
        self.res_block5 = ConvNeXtV2tiny(self.inplanes,self.inplanes,down=True)
    ...
    def forward(self, x ,cx16):
        ...
        return pred6
class Tiny_Unetlikev6(nn.Module):
    def __init__(self, inplanes, outplanes,mid_channle = 128):
        super(Tiny_Unetlikev6, self).__init__()
        self.proj0 = conv2DBatchNormRelu(in_channels=inplanes, k_size=1, n_filters=mid_channle, padding=0, stride=1)
        self.inplanes = mid_channle
        # Vanilla Residual Blocks
        self.res_block1 = ConvNeXtV2tiny(self.inplanes,int(self.inplanes*0.5),down=True)#1/2
        self.res_block2 = ConvNeXtV2tiny(int(self.inplanes*0.5),int(self.inplanes*0.5),down=True)#1/4
        self.res_block3 = ConvNeXtV2tiny(int(self.inplanes*0.5), self.inplanes, down=True)#1/8
        self.res_block4 = ConvNeXtV2tiny(self.inplanes, self.inplanes, down=True)#1/16
        ...
        self.upconv6 = conv2DBatchNormRelu(in_channels=self.inplanes, k_size=3, n_filters=self.inplanes,
                                           padding=1, stride=1)
        self.upconv5 = conv2DBatchNormRelu(in_channels=self.inplanes, k_size=3, n_filters=self.inplanes,
                                           padding=1, stride=1)
        self.upconv4 = conv2DBatchNormRelu(in_channels=int(self.inplanes*0.5), k_size=3, n_filters=int(self.inplanes*0.5),
                                           padding=1, stride=1)
        self.upconv3 = conv2DBatchNormRelu(in_channels=self.inplanes, k_size=3, n_filters=96,
                                           padding=1, stride=1)


        self.iconv5 = conv2DBatchNormRelu(in_channels=self.inplanes*2, k_size=3, n_filters=self.inplanes,
                                          padding=1, stride=1)
        self.iconv4 = conv2DBatchNormRelu(in_channels=int(self.inplanes*1.5), k_size=3, n_filters=int(self.inplanes*0.5),
                                          padding=1, stride=1)
        self.iconv3 = conv2DBatchNormRelu(in_channels=self.inplanes * 3, k_size=3, n_filters=self.inplanes,
                                          padding=1, stride=1)
        self.iconv2 = conv2DBatchNormRelu(in_channels=self.inplanes+96, k_size=3, n_filters=self.inplanes,
                                          padding=1, stride=1)
    ...
    def forward(self, x ,cx16):
        proj = self.proj0(x)  # 4x  1
        conv1 = self.res_block1(proj)  # 8x  1/2
        conv2 = self.res_block2(conv1)  # 8x  1/4
        conv3 = self.res_block3(conv2)  # 8x  1/8
        conv6 = self.res_block4(conv3)  # 8x  1/16

        conv6 = self.pyramid_pooling(conv6)  # 1/16

        conv6u = F.upsample(conv6, [conv3.size()[2], conv3.size()[3]], mode='bilinear')
        concat5 = torch.cat((conv3, self.upconv6(conv6u)), dim=1)
        conv5 = self.iconv5(concat5)  # 1/8

        conv5u = F.upsample(conv5, [conv2.size()[2], conv2.size()[3]], mode='bilinear')
        concat4 = torch.cat((conv2, self.upconv5(conv5u)), dim=1)
        conv4 = self.iconv4(concat4)  # 1/4

        conv4u = F.upsample(conv4, [conv1.size()[2], conv1.size()[3]], mode='bilinear')
        concat3 = torch.cat((cx16,conv1, self.upconv4(conv4u)), dim=1)
        conv3i = self.iconv3(concat3)  # 1/2

        conv3u = F.upsample(conv3i, [x.size()[2], x.size()[3]], mode='bilinear')
        concat2 = torch.cat((proj, self.upconv3(conv3u)), dim=1)
        conv2i = self.iconv2(concat2)
        pred6 = self.proj6(conv2i)  # 4x

        return pred6

#这个东西前面配合初始化好的Net,用1/16的特征初始化
class Tiny_Unetlikev9(nn.Module):
    def __init__(self, hidden_dim=128, input_dim=256):
        super(Tiny_Unetlikev9, self).__init__()

        self.convz =  ConvNeXtV2tiny(hidden_dim+input_dim, hidden_dim, kszie=5, bl=2)
        self.convr =  ConvNeXtV2tiny(hidden_dim+input_dim, hidden_dim, kszie=5, bl=2)
    ...
    def forward(self, h, x):
        ...
        q = torch.tanh(self.convq(torch.cat([r*h, x], dim=1)))#更新量占多少比重
        h = (1-z) * h + z * q#旧的按照比例加上新的

...
def forward_interpolate(flow,expu):
    flow = flow.detach().cpu().numpy()
    expu = expu.detach().cpu().numpy()
    dx, dy = flow[0], flow[1]
    dexp = expu[0,0]
    ht, wd = dx.shape
    ...
    dc = interpolate.griddata(
        (x1, y1), dexp, (x0, y0), method='nearest', fill_value=0)
    flow = np.stack([flow_x, flow_y], axis=0)
    return torch.from_numpy(flow).float(),torch.from_numpy(dc).float()
...
def coords_gridsea(batch, ht, wd, device):
...
def upconf8(conf, mode='bilinear'):
...
def updepth2(flow, mode='bilinear'):
    new_size = (2 * flow.shape[2], 2 * flow.shape[3])
